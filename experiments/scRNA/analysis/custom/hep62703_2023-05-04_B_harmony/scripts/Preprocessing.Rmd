...
params$analparams <- proot$find_file("experiments/scRNA/analysis/custom/hep62703_2023-05-04_B_harmony/analparams.yaml")
...
loadndemux<-function(datdir){
    ...
    # Fix issue with wrong sample names in antibody capture matrix (Ctrl.1 instead of Ctrl)

    rownames(matrices$`Antibody Capture`) <- stringr::str_replace(rownames(matrices$`Antibody Capture`), "Ctrl\\.1", "Ctrl")
