...
diffExpression <- readRDS(proot$find_file(config$outputpath, "data", "stats_nebula_cond_cpc-0.005_2023-05-10.rds"))
...
```{r prepare_genesets, eval=T, echo=F}
...
for (celltype in names(rankedGenes)) {
    print(Sys.time())
    print(paste0("Celltype: ", celltype))
    fgseaResults[[celltype]] <- list()
    for (comparison in names(rankedGenes[[celltype]])) {
        print(Sys.time())
        print(paste0("Comparison: ", comparison))
        fgseaResults[[celltype]][[comparison]] <- list()
        currGenes <- rankedGenes[[celltype]][[comparison]]
        for (geneSetLib in names(msigdbSetsEffSubset[[celltype]])) {
            print(paste0("Msigdb gene set library: ", geneSetLib))
            fgseaResults[[celltype]][[comparison]][[geneSetLib]] <- as.data.frame(
                fgsea(
...
saveRDS(
    ...
    proot$find_file(config$outputpath, "data", paste0("gsea_nebula_cond_cpc-0.005_", today, ".rds"))
)
...
for (celltype in celltypes) {
    ...
    currFilename <- paste0(
        "gsea_nebula_cond_cpc-0.005_", celltype, "_", today, ".xlsx"
    )
