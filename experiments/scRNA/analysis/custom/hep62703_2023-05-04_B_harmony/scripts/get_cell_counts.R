...
relevantCelltypes <- c("Stem", "TA", "Enterocyte.Progenitor", "Enterocyte", "Endocrine", "Goblet.Paneth", "Tuft")
...
groups <- sort(unique(countsLong$group))

countsLong <- countsLong %>%
    mutate(celltype = factor(celltype, levels = relevantCelltypes)) %>%
    mutate(group = factor(group, levels = sort(unique(countsLong$group)))) %>%
...
write.csv(
    countsPivot,
    proot$find_file(config$outputpath, "data", paste0("celltype_counts_", today, ".csv")),
    quote = FALSE,
    row.names = FALSE
)